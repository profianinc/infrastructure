...
with flake-utils.lib.system; let
  mkSteward = self.lib.mkService [
    ...
    ({
      ...
      systemd.services.steward.serviceConfig.ExecStartPre = "+${self.lib.exposeKey pkgs "steward" config.sops.secrets.key.path}";
      systemd.services.steward.serviceConfig.ExecStop = "+${self.lib.hideKey pkgs config.users.users.root.name config.sops.secrets.key.path}";
